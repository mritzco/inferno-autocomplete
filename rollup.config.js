...
let plugins = [
	...
	commonjs({
		...
		exclude: ['node_modules/symbol-observable/**', '**/*.css', '**/*.json', 'package.json']
	})
];
...
function createBundle({ moduleGlobal, moduleName, moduleEntry, moduleGlobals }, path) {
	...
	const copyright =
		...
		' */';
	const entry = './lib/inferno-autocomplete';
	const dest  = pack.main;
...
Promise.all(['Autocomplete'].map(bundle => createBundle(bundle, 'dist/')));
