...
export default class Autocomplete extends Component {
  ...
  selectItemFromMouse (item) {
    ...
    var input = document.getElementById(this.refs.id);
    input.focus()
    // Itzco: This isn't set.
  ...
  renderMenu () {
    ...
    var items = this.getFilteredItems().map((item, index) => {
      ...
      var el = React.cloneElement(element, {
        onMouseDown: () => this._ignoreBlur = true, //this.setIgnoreBlur(true).bind(instance), // Ignore blur to prevent menu from de-rendering before we can process click
    ...
    var menu = (this.props.renderMenu || function renderMenu(items, value, style) {
      return createElement('div',
          {
            style: { ...style,  ...instance.state.menuStyle, ...instance.props.menuStyle},
            className:"autoCompleteOptions",
            id: 'opts_' + instance.props.inputProps.id, /* aria-controls */
            children: items
          }
        );
  ...
  render () {
    const { inputProps, wrapperStyle, wrapperProps } = this.props
    const controls = 'opts_' + inputProps.id;
